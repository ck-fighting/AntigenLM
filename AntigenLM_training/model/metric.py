...
#     def compute_metrics(self, eval_pred, top_n=3):
#         print("🚀 compute_metrics 被调用啦！！！")

#         labels = eval_pred.label_ids  # Shape: (batch_size, seq_len)
#         preds = eval_pred.predictions # Shape: (batch_size, seq_len, vocab_size)

#             # 若需要 squeeze()，看实际数据维度而定
#             # 例如 labels = labels.squeeze()，如果labels本身是 (n, 32)
#             # 也可以先检查维度
#         if labels.ndim == 3 and labels.shape[-1] == 1:
#             # 例如 (batch_size, seq_len, 1) => squeeze掉最后一维
...
#             # 查找哪些位置是真正的mask/需要预测的( != -100)
#             masked_idx = np.where(labels[i] != -100)[0]
#             n_mask = len(masked_idx)
#             n_mask_total += n_mask
#             pred_arr = preds[i, masked_idx]  # [n_mask, vocab_size]
#             truth = labels[i, masked_idx]    # [n_mask]

#                 # argsort: 沿 vocab 轴从小到大排序 => [:, -1] 是 top1
#             pred_sort_idx = np.argsort(pred_arr, axis=1)
#                 # top1:
...
#                 if truth_idx in top_n_idx:
#                     top_n_correct += 1
#                     # 如果需要 BLOSUM 打分
#                 if self.blosum:
#                     truth_res = self.token_with_special_list[truth_idx]
#                     pred_res  = self.token_with_special_list[top_n_idx[-1]]  # top1
#                         # 这里假设 truth_res 和 pred_res 都是单字符
#                         # 如果它们可能是多字符，可以再加层循环
#                     if truth_res in self.BLOSUM.index and pred_res in self.BLOSUM.index:
#                         blosum_values.append(self.BLOSUM.loc[truth_res, pred_res])

#             # 避免分母0
#         if n_mask_total == 0:
#             return {f"top_{top_n}_acc": 0.0, "acc": 0.0, "average_blosum": 0.0}
...
#                 "average_blosum": np.mean(blosum_values) if len(blosum_values) > 0 else 0.0
#             }
#         else:
#             return {
#                 f"top_{top_n}_acc": top_n_correct / n_mask_total,
...
#     def compute_metrics_fine(self, eval_preds):
#             logits, labels = eval_preds  # Hugging Face 自动传入 (logits, labels)

#             # Step 1: 取预测值（最大概率的 token）
#             predictions = np.argmax(logits, axis=-1)  # shape: [batch_size, seq_len]

#             # Step 2: 只在 labels != -100 的位置评估
#             mask = labels != -100
#             correct = (predictions == labels) & mask
#             acc = correct.sum() / mask.sum()

#             # Step 3: 可选的 BLOSUM 分数（需要你实现 get_blosum_score）
#             # blosum_score = self.get_blosum_score(predictions, labels, mask)

#             return {
#                 "eval_acc": acc.item(),
#                 # "eval_blosum": blosum_score,  # 如果你有 BLOSUM 分数就加上
#             }

class MAA_metrics:
    def __init__(self, token_with_special_list, blosum_dir=None, blosum=False):
        self.token_with_special_list = token_with_special_list
        self.blosum = blosum
        self.AMINO_ACIDS = "ACDEFGHIKLMNPQRSTUVWY"
        self.BLOSUM = self._load_blosum(blosum_dir) if blosum and blosum_dir else None

    def _load_blosum(self, path):
        with open(path) as source:
            d = json.load(source)
        df = pd.DataFrame(0, index=list(self.AMINO_ACIDS), columns=list(self.AMINO_ACIDS))
        for x, y in itertools.product(df.index, df.columns):
            if x != "U" and y != "U":
                df.loc[x, y] = d[x][y]
        return df
    ...
    def compute_metrics(self, eval_pred, top_n=3):
        ...
        for i in range(labels.shape[0]):
            masked_idx = np.where(labels[i] != -100)[0]
            n_mask_total += len(masked_idx)
            pred = logits[i, masked_idx]
            truth = labels[i, masked_idx]

            top_preds = np.argsort(pred, axis=1)[:, -top_n:]

            for true_idx, top_idxs in zip(truth, top_preds):
                if true_idx == top_idxs[-1]:
                    top1_correct += 1
                if true_idx in top_idxs:
                    topn_correct += 1
                if self.blosum:
                    true_aa = self.token_with_special_list[true_idx]
                    pred_aa = self.token_with_special_list[top_idxs[-1]]
                    if true_aa in self.BLOSUM.index and pred_aa in self.BLOSUM.columns:
                        blosum_scores.append(self.BLOSUM.loc[true_aa, pred_aa])

        if n_mask_total == 0:
            return {
                "eval_acc": 0.0,
                f"eval_top{top_n}_acc": 0.0,
                "eval_blosum": 0.0,
            }

        return {
            "eval_acc": top1_correct / n_mask_total,
            f"eval_top{top_n}_acc": topn_correct / n_mask_total,
            "eval_blosum": np.mean(blosum_scores) if blosum_scores else 0.0,
        }
...
def squared_error_zero(y_pred, y_true, eps=1e-12):
    """
    计算强制过原点的 r0^2。
    1) 先基于 (y_pred, y_true) 拟合无截距斜率 beta
    2) 用 beta*y_pred 做新的预测
    3) r0^2 = 1 - SSE / SST， 其中 SSE=∑(y_true - beta*y_pred)^2, SST=∑(y_true^2)
    """
    ...
    denominator = np.sum(y_pred * y_pred) + eps
    beta = numerator / denominator

    # 2) 新的预测
    y_pred_forced = beta * y_pred

    # 3) 计算 SSE 和 SST
    sse = np.sum((y_true - y_pred_forced)**2)
    sst = np.sum(y_true**2) + eps
