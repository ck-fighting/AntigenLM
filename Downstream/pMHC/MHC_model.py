...
class Cross_Attention(nn.Module):
    ...
    def forward(self, pep_inputs, hla_inputs):
        ...
        dec_self_attn_pad_mask = torch.LongTensor(
            np.zeros((pep_inputs.shape[0], pep_inputs.shape[1], hla_inputs.shape[1]))
        ).bool().to(device)
        dec_self_attns = []
        for layer in self.layers:
...
class Mymodel_HLA(nn.Module):
    ...
    def forward(self, pep_emb, hla_input):
        hla_enc, hla_attn = self.encoder_H(hla_input)
        pep = self.pep_proj(pep_emb)
        pep2hla, attn1 = self.cross(pep, hla_enc)                  # [B, Lp, D]
...
#     def forward(self, pep_emb, hla_input):
#         hla_enc, hla_attn = self.encoder_T(hla_input)              # [B, Lh, D]
#         pep = self.pep_proj(pep_emb)                               # [B, Lp, D]  (Lp 可能是 512)
#         pep2hla, attn1 = self.cross(pep, hla_enc)                  # [B, Lp, D]

#         # === 关键：把序列长度自适应压到 pep_max_len ===
#         # [B, Lp, D] -> [B, D, Lp] -> 自适应池化到 [B, D, pep_max_len] -> [B, pep_max_len, D]
#         x = pep2hla.transpose(1, 2)
