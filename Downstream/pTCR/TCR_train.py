...
def _model_id_for_cache(cfg) -> str:
    ...
    if cfg.embed_backend == "antigenLM_withoutSS":
        if cfg.antigenLM_antigenLM_withoutSS_path:
            return os.path.basename(cfg.antigenLM_antigenLM_withoutSS_path.rstrip("/"))
        return "antigenLM_withoutSS"
    if cfg.embed_backend == "antigenLM_withoutSlidingwindow":
        if cfg.antigenLM_withoutSlidingwindow_path:
            return os.path.basename(cfg.antigenLM_withoutSlidingwindow_path.rstrip("/"))
...
def extract_peptide_embeddings(
    ...
    if cfg.embed_backend == "esm2":
    ...
    elif cfg.embed_backend == "antigenLM_withoutSlidingwindow":
        return antigenLM_extract(
            pep_list,
            model_name_or_path=cfg.antigenLM_withoutSlidingwindow_path,
            device=device,
            max_len=pep_max_len if cfg.embed_max_len_override <= 0 else cfg.embed_max_len_override
        )
        ...
        return antigenLM_extract(
            pep_list,
            model_name_or_path=cfg.antigenLM_antigenLM_withoutSS_path,
            device=device,
            max_len=pep_max_len if cfg.embed_max_len_override <= 0 else cfg.embed_max_len_override
        )
...
class TCRDataset(Dataset):
    """Minimal tensor dataset: returns (pep_embed, tcr_ids, label)."""
    ...
    def __getitem__(self, idx):
        return self.pep[idx], self.tcr[idx], self.y[idx]
...
def data_process_TCR(
    ...
    # Clean empty rows so peptide/TCR counts always align
    data = data.copy()
    ...
    if is_main_process():
        if not os.path.exists(cache_path):
            ...
            # Cache may come from a different dataset; guard by length
            ...
            if pep_embeddings.shape[0] != len(pep_list):
                ...
                torch.save(pep_emb.detach().cpu(), cache_path)
                log(f"[Cache] Overwrote peptide embeddings -> {cache_path}")
    ...
    # —— TCR tokenization & labels（保持你原来的逻辑） —— 
    try:
    ...
    for tcr_seq, y in zip(data.TCR, data.label):
        padded = tcr_seq.ljust(_tcr_max_len, "-")
...
def train_one_epoch(
    ...
    for batch in train_loader:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            logits, _, pep_tcr = model(pep, tcr)  # logits: [B,1] or [B]
            logits = logits.view(-1)
            main_loss = bce(logits, labels.float())

            # SupCon on pooled features
            # feats = pep_tcr.mean(dim=1)  # [B, D]
            s_loss = supcon(pep_tcr, labels)
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            s_loss_adv = supcon(pep_tcr_adv.mean(dim=1), labels)
...
def parse_args():
    p = argparse.ArgumentParser(description="DDP training for pTCR binding (clean version)")
    p.add_argument("--cv_dir", type=str, default="./data/cv_LLM", help="Directory with train_fold_*.csv & val_fold_*.csv")
    p.add_argument("--emb_cache_dir", type=str, default="./data/data_TCR_cached/Ablation", help="Directory to cache peptide embeddings")
    ...
    p.add_argument("--save_dir", type=str, default="../trained_model/TCR_micro/Ablation")

    p.add_argument("--embed_backend", type=str, default="antigenLM_withoutSlidingwindow", choices=["esm2","esmc","antigenLM","antigenLM_withoutSS","antigenLM_withoutSlidingwindow","microLM","PathogLM"])
    ...
    p.add_argument("--antigenLM_path", type=str, default="../../LLM/AntigenLM_300M_withoutSS")
    p.add_argument("--antigenLM_antigenLM_withoutSS_path", type=str, default="../../Other_LLM/AntigenLM_300M_withoutSS")
    p.add_argument("--antigenLM_withoutSlidingwindow_path", default="/home/dataset-local/AntigenLM/Other_LLM/AntigenLM_300M_withoutSlidingwindow")
