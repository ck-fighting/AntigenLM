...
class SelfSupervisedDataset(Dataset):
    def __init__(
        ...
        self.space_separated_input = space_separated_input
    ...
    def __getitem__(self, index):
        ...
        tokens = self.split_fun(seq)
        token_text = self._insert_whitespace(tokens) if self.space_separated_input else "".join(tokens)
    ...
    def merge(self, other):
        ...
        return SelfSupervisedDataset(
            ...
            self.max_len,
            self.logger,
            space_separated_input=self.space_separated_input,
        )
...
class MAADataset:
    def __init__(self, config, logger, seed, seq_dir, tokenizer_name, vocab_dir, token_length_list, seq_name, max_len=None, test_split=0.1):
        ...
        self.use_esmc_tokenizer = tokenizer_name.lower() == "esmc"
        self.space_separated_input = not self.use_esmc_tokenizer
        if self.use_esmc_tokenizer:
            self.bert_tokenizer = self._build_esmc_tokenizer()
            self.split_fun = list
            self.tokenizer = None
        else:
        ...
        if not self.use_esmc_tokenizer:
    ...
    def _load_seq(self, seq_dir):
        """
        从 FASTA/TXT 文件或带 sequence 列的 CSV 文件中加载序列。
        """
        seq_list = []
        if seq_dir.endswith(".csv"):
            with open(seq_dir, "r", newline="") as f:
                reader = csv.DictReader(f)
                if "sequence" not in (reader.fieldnames or []):
                    raise ValueError(f"CSV file {seq_dir} must contain a 'sequence' column.")
                for row in reader:
                    seq = (row.get("sequence") or "").strip()
                    if seq:
                        seq_list.append(seq)
            self.logger.info(f"Loaded {len(seq_list)} sequences from CSV {seq_dir}.")
            return seq_list
    ...
    def get_token_list(self):
        if self.use_esmc_tokenizer:
            tokens = self.bert_tokenizer.convert_ids_to_tokens(list(range(len(self.bert_tokenizer))))
            tokens.extend(f"<unused_{idx}>" for idx in range(len(tokens), self.get_vocab_size()))
            return tokens
    ...
    def get_pad_token_id(self):
        if self.use_esmc_tokenizer:
            return self.bert_tokenizer.pad_token_id
